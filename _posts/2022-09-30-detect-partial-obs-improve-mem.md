...
The content of this article comes from work done at the [Intelligent Robot Lab](http://irl.cs.brown.edu/) at Brown University during the summer of 2022 alongside [Cam Allen](https://camallen.net/), [Sam Lobel](https://samlobel.github.io/), [Saket Tiwari](https://tsaket.github.io/), Anita de Mello Koch, [David Tao](http://taodav.cc/), [Omer Gottesman](https://cs.brown.edu/people/faculty/ogottesm/), [Michael Littman](https://www.littmania.com/), and [George Konidaris](https://cs.brown.edu/people/gdk/). We explore a method aimed at reliably detecting aliasing in POMDPs and using this signal to search for memory functions that allow for finding higher performing policies--all without previous knowledge of the state-space.
...
We will begin by adding some formalism to our simple example, starting with a fully observable case as a baseline. The sequential decision-making problem in our example can be modeled as a Markov Decision Process (MDP) as shown in Figure 2, named Environment A. There, the colors indicate the observation seen at that state (which we can think of as the wall colors in the simple example). In Environment A, the observations uniquely identify the underlying states, so the environment is fully observable. The optimal policy can be attained only by looking at the *current* color and selecting an action based on that. In this environment, any policy where $\pi(red)=DOWN$ and $\pi(green)=UP$ is optimal and will result in the agent obtaining the maximum possible reward of +2.

Now, contrast this with Environment B in Figure 3 which shows a new environment that is very similar to Environment A except that there are two states that look the same (i.e. have the same wall color) so we consider Environment B according to the Partially Observable MDP (POMDP) framework. Both environments are Markovian in the sense that the underlying dynamics depend only on information from the current timestep. However, in Environment B, the best memory-based policy can do better than the best memoryless policy due to the aliasing resulting from the observation function behavior. In particular, to get the maximum possible reward in this environment (+2), one needs to keep track of history to know what to do upon seeing a red observation. Is the agent at the *first* red-looking state? Then take DOWN and get +1. Or the *second* one? Then take UP and get +1.  
...
We've seen that the agent cannot detect aliasing by looking at the observations alone as we execute the policy. We need something else that the agent can latch on to as it traverses the (PO)MDP--something that it can use as a signal of aliasing and thus partial observability so that it can eventually learn a policy that achieves the +2 return. Often in RL, an intermediate step in finding the optimal policy is estimating the value of states, $V(s)$, or state-action pairs, $Q(s,a)$. In the POMDP case, these (and the policy) are considered to be over observations: $V(o)$ and $Q(o,a)$. For any given observation-action pair, the $Q$ value gives the expected cumulative discounted reward that will be received starting from this observation-action pair and proceeding from here to the end of the episode according to the given policy. The values are useful in iterating toward the optimal policy because we would like to find the policy that achieves the highest expected return, and this expected return is what the values represent. 

One natural method to estimate these values is to run the policy in question on the POMDP and record what the average total discounted returns are from each observation-action pair in a Monte-Carlo (MC) fashion. This is a natural algorithm to apply because it uses the real rewards seen over the course of the interaction to generate an expectation of what return will be achieved from any point in the POMDP. This is particularly important for accuracy in POMDPs because, while the agent in executing its policy is only making decisions with observations, its actions on the environment still move it within the underlying state space, and MC records the reward outcomes of this movement without any bias (i.e. without intermediate estimations) and applies them to the actual observation seen at that state.

However, the degree of bias can be tuned from here using the [TD($\lambda$)](http://incompleteideas.net/papers/sutton-88-with-erratum.pdf) algorithm wherein a $\lambda$ value of 1 provides results equivalent to MC and lower values of $\lambda$ dial up the bias.



[TD($\lambda$)](http://incompleteideas.net/papers/sutton-88-with-erratum.pdf) is an algorithm for estimating these values by repeatedly running through the environment from beginning to end according to a policy and updating the estimation until convergence. In the case of TD(1), the value of an observation is estimated to be the discounted sum of all of the actual rewards seen after visiting this observation, averaged over all trajectories. In TD(0), the value of an observation is estimated to be the immediate real reward upon leaving a state plus the current estimated value of the next observation (a.k.a. bootstrapping off of the next observation). Intermediate $\lambda$ values form a spectrum between these behaviors, but we will consider these two $\lambda$ values here.

The bootstrapping mechanism is the basis for the hypothesis that **running TD($\lambda$) with different values of $\lambda$ over the observation space of a given environment provides a signal of memory-improvable partial observability**. Namely, if the $Q$ values of any observation-action pair estimated by, say, TD(0) and TD(1) differ--in other words, there is a non-zero **$\lambda$-discrepancy**--then at least one observation must have been emitted by multiple underlying states because TD(0) during bootstrapping will have been erroneously sharing values of states that emitted the same observation. The assumption of sub-problem composability underlying the Bellman equation has been broken. In an MDP, however, TD evaluations with different $\lambda$ values eventually all converge to the same result.

To explore the theory, we can analytically determine the values that these two variations of the TD algorithm will converge to in expectation given the full set of quantities comprising the environment model. This means we know the finite set of possible states $S$, the finite set of possible actions $A$, the finite set of possible observations $Z$, and all of the POMDP quantities listed in Section 1. First, we calculate the values of the states of the underlying MDP using the following system of linear equations:
...
The idea is to aggregate state-state relationships within the POMDP dynamics into observation-observation relationships. Aggregating $T$ and $R$ requires the same $P_\pi(s\|o)$ object to weight contributions but we are operating in higher dimensions now. Both the current and next observations must be considered so the pseudocode shows how it would work with two `for` loops and maintaining the actions dimension: 
...
This memory function actually fully resolves the partial observability in the environment which may not always be possible (in those cases some value improvement still may be possible). The agent can now observe both the observation from the environment and the state of its memory function when making decisions. During an episode, instead of `red -> blue -> red -> purple` it will now see `red+M0 -> blue+M0 -> red+M1 -> purple+M0`. Importantly, the two states that looked the same (both red), now look different (`red+M0` vs. `red+M1`). The agent can now learn about them separately and make unique decisions.
